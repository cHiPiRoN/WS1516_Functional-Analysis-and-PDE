...
% Think about...
	...
	\begin{proof}
		...
		We can now define an equivalence relation $\mathtt{\sim}$ by 
		\[
			x\mathtt{\sim} y \,\text{if } \lim_{j\to\infty} d(x_j,y_j) = 0
		\] 
		Then define the set of equivalence classes as
		\[
			\tilde{X} = \hat{X}/_{\mathtt{\sim}} = \{[x]\}
		\]
		with
		\[
			[x] \coloneqq \{y\in\hat{X}|\,x\mathtt{\sim}y\}.
		\]
